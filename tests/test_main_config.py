import logging
from main import get_config  # Replace 'main' with the module name
...
def test_get_config_with_custom_path(caplog, mocker):
    ...
    with patch("main.open", mock_open_file), caplog.at_level(logging.INFO):
        # Mock tomllib to simulate loading the configuration dictionary
        mock_tomllib = mocker.patch("main.tomllib.load", return_value={"key": "value"})
...
def test_get_config_with_default_path(caplog, mocker):
    ...
    with patch("main.open", mock_open_file), caplog.at_level(logging.INFO):
        ...
        mock_tomllib = mocker.patch(
            "main.tomllib.load", return_value={"key": "default_value"}
        )
